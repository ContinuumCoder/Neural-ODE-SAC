...
def constraint_fn(u):
    ...
    energy = 0.5 * torch.sum(U**2 + V**2, dim=1)
    diff = 6.1 - energy
...
#  
#     target_energy = 6.1
#     distance_to_manifold = torch.sqrt(torch.abs(energy - target_energy))
...
def manifolf_constraint_fn(u):
    ...
    target_energy_lower_bound = 6.1    
    distance_to_manifold = torch.relu(target_energy_lower_bound - energy)
...
#     u_reshaped = u.reshape(bs, 2, n)
#     U, V = u_reshaped[:, 0, :], u_reshaped[:, 1, :]
#     energy = torch.sum(U**2 + V**2, dim=1)
#     return 6.1 - energy
...
def conservation_loss(predicted_seq):
    ...
    conservation_value_seq = (u_seq ** 2 + v_seq ** 2) * 0.5
...
def eval_model(model, model_name, vis_seq_index = 1, vis_ts=60):
    ...
    model.eval()
    test_uv_data = np.load(data_directory + "/test_data.npy")
    ...
    long_test_uv_data = np.load(data_directory + "/long_test_data.npy")
...
if __name__ == '__main__':
    ...
    uv_data = np.load(data_directory + "/data.npy")
