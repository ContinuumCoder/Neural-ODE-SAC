...
def conservation_loss(predicted_seq):
    ...
    conservation_value_seq = (u_seq + v_seq)
...
def eval_model(model, model_name, vis_seq_index = 1, vis_ts=60):
    ...
    model.eval()
    test_uv_data = np.load(data_directory + "/LK_test_data.npy")
    ...
    long_test_uv_data = np.load(data_directory + "/LK_long_test_data.npy")
...
def constrain_level(model, model_name):
    ...
    predicted_states = predicted_states.cpu().detach().numpy().transpose(1, 0, 2).reshape(batch_size, time_step, 2, -1)
    ...
    for traj in predicted_states:
        ...
        for t in range(time_step):
            u, v = traj[t]
            energy = compute_active_level(torch.from_numpy(u), torch.from_numpy(v))
...
if __name__ == '__main__':
    ...
    uv_data = np.load(data_directory + "/LK_data.npy")
    ...
    if not EVAL_MODE:
        ...
        node_sac_m_nl_mse_l, node_sac_m_nl_l1_l, node_sac_m_nl_t_mse_l, node_sac_m_nl_t_l1_l = trainer.train(node_sac_m_nl, "NODE_SAC_MANIFOLD_CONSTRAIN_N0_LOSS", node_sac_m_nl_path)
    ...
    node_sac_constrain_level = constrain_level(node_sac_m_nl, "NODE_SAC_MANIFOLD_CONSTRAIN_NO_LOSS")
    ...
    # plt.ylabel('Active Level')
    # plt.title('Comparison of Active Levels for Different Models')
    # plt.legend()
    # plt.tight_layout()
    # # plt.show()
    ...
    fig.update_layout(
        title=dict(
            text='Active Levels for Different Methods in Lotka-Volterra Modeling',
        ...
        yaxis=dict(
            title=dict(text='Active Level', font=dict(family='Times New Roman', size=22, color='black')),
            tickfont=dict(family='Times New Roman', size=18, color='black')
        ),
